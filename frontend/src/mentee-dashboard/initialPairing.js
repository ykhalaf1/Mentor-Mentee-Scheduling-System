...
const TOTAL_POSSIBLE_SCORE = Object.values(WEIGHTS).reduce(
...
const calculatePartialMatch = (mentorText, menteeText) => {
  ...
  mentorWords.forEach((word) => {
    if (
      menteeWords.some(
        (menteeWord) => menteeWord.includes(word) || word.includes(menteeWord)
      )
    ) {
      matches++;
    }
...
const calculateIndustryScore = (mentorIndustry, menteeIndustry) => {
  ...
  const mentorIndustries = Array.isArray(mentorIndustry)
    ? mentorIndustry
    : [mentorIndustry];
  const menteeIndustries = Array.isArray(menteeIndustry)
    ? menteeIndustry
    : [menteeIndustry];

  const commonIndustries = findCommonElements(
    mentorIndustries,
    menteeIndustries
  );
  ...
  if (commonIndustries.length > 0) {
    return (
      ...
      (commonIndustries.length /
        Math.max(mentorIndustries.length, menteeIndustries.length))
    );
  }
...
const calculateSkillsScore = (mentorSkills, menteeSkills) => {
  ...
  const mentorSkillsString =
    typeof mentorSkills === "string" ? mentorSkills : String(mentorSkills);
  const mentorSkillsArray = Array.isArray(mentorSkills)
    ? mentorSkills
    : mentorSkillsString.split(",").map((s) => s.trim());
  const menteeSkillsArray = Array.isArray(menteeSkills)
    ? menteeSkills
    : menteeSkills.split(",").map((s) => s.trim());
...
const calculateHelpInScore = (mentorHelpIn, menteeServiceLookingFor) => {
  ...
  const mentorHelpInString =
    typeof mentorHelpIn === "string" ? mentorHelpIn : String(mentorHelpIn);
  const mentorServices = mentorHelpInString
    .split(",")
  ...
  // Partial match
  const partialMatch = calculatePartialMatch(
    mentorHelpInString,
    menteeServiceLookingFor
  );
  if (partialMatch > 0.3) {
...
const calculateCompanySizeScore = (
  mentorCompanySize,
  menteeCompanySizePreference
) => {
  if (!mentorCompanySize || !menteeCompanySizePreference) return 0;

  const mentorSizes = Array.isArray(mentorCompanySize)
    ? mentorCompanySize
    : [mentorCompanySize];
  const menteePreferences = Array.isArray(menteeCompanySizePreference)
    ? menteeCompanySizePreference
    : [menteeCompanySizePreference];
  ...
  if (commonSizes.length > 0) {
    return (
      WEIGHTS.COMPANY_SIZE * (commonSizes.length / menteePreferences.length)
    );
  }
...
const calculateTotalScore = (mentor, mentee) => {
  const scores = {
    ...
    companySize: calculateCompanySizeScore(
      mentor.companySizeExperience,
      mentee.companySizePreference
    ),
  };
...
const findTopMentors = (mentee, mentors, limit = 3) => {
  const scoredMentors = mentors.map((mentor) =>
    calculateTotalScore(mentor, mentee)
  );
...
export { findTopMentors, calculateTotalScore, WEIGHTS, TOTAL_POSSIBLE_SCORE };
